...
%ADAPTADO 22/09, solução ALETAS

%m=> y linhas,  n=> x colunas
...
%% Balanço para primeira parede

%Condução T prescrita nas quinas
cond1 = 0;
...
%Confere se T é prescrita
if (data.state_1 == 1)
    for j=2:n-1
         cond1=cond1+k*dx*(T(1,j)-T(2,j))/dx;
    end
    conv1 = 0;
%Caso não seja prescrita
else
    %Quinas sem temperatura prescrita
    conv1 = 0;
    if (data.state_2 ~= 1)
